 ...
 * const pass = 'password';  
 * 
 * const conection = 'mongodb://'+ user +':'+ pass +'@url:port';
 * For example:
 * const conection = 'mongodb://'+ user +':'+ pass +'@192.168.2.1:27017';
 *
 */
...
//*** Conection to DB */
    ...
    const client = MongoClient.connect(uriDB,{useNewUrlParser: true, useUnifiedTopology: true})
    .then(
        client => 
        {            
            db = client.db('events');
    ...
    router.get('/', async(req, res) => {
            ...
            if(typeof req.query.id !== 'undefined' &&  req.query.id !== 'false') {
                //res.send("9999999");
                JsonArray1 = await collection.find({_id: new mongodb.ObjectID(req.query.id) }).toArray() 
